(function () {
    ...
    var ASSETS = ASSETS_BASE.slice();

    var PRIZE_BY_SRC = (function () {
        var m = {};
        m["assets/GULA.png"]    = "Gula";
        m["assets/KALSIUM.png"] = "Kalsium";
        m["assets/LEMAK.png"]   = "Lemak Trans";
        m["assets/MINERAL.png"] = "Mineral";
        m["assets/NATRIUM.png"] = "Natrium";
        m["assets/OMEGA.png"]   = "Omega 3&6";
        m["assets/PERASA.png"]  = "Perasa Buatan";
        m["assets/VITAMIN.png"] = "Vitamin";
        m["assets/ZINC.png"]    = "Zinc";
        return m;
    })();
    ...
    /* KALSIUM, OMEGA, MINERAL, VITAMIN, ZINC = nutrisi baik → end_card.jpg */
    var WIN_ITEMS = ["KALSIUM", "OMEGA", "MINERAL", "VITAMIN", "ZINC"];
    ...
    function initPrizes() {
        ...
        var half = side / 2;
        /* Tanpa bubble: minDist = side item agar saling bersentuhan (sedikit overlap ok) */
        var minDist = side * 0.88;

        /* Batas area dalam koordinat PUSAT item */
        var cxMin = pitPad + half;
        var cxMax = W - pitPad - half;
        var cyTop = railY + 50 + half;
        var cyBot = floorY - half - 2;

        /*
         * Pisahkan item_phone ke posisi paling bawah (ditempatkan terakhir di cyBot).
         * Semua item lain diacak normal.
         */
        /* GULA & LEMAK ditempatkan pertama → paling bawah (paling susah dicapai) */
        var hardItems = images.filter(function (img) {
            return img.src.indexOf("GULA") !== -1 || img.src.indexOf("LEMAK") !== -1;
        });
        var otherItems = images.filter(function (img) {
            return img.src.indexOf("GULA") === -1 && img.src.indexOf("LEMAK") === -1;
        });
        shuffleInPlace(otherItems);
        var pool = hardItems.concat(otherItems);
        var n = pool.length;
        ...
        for (pi = 0; pi < n; pi++) {
            var isPhone = pool[pi].src.indexOf("GULA") !== -1 || pool[pi].src.indexOf("LEMAK") !== -1;
            ...
            for (t = 0; t < scanSteps; t++) {
                ...
                /* GULA/LEMAK: selalu paling bawah (susah dicapai) */
                /* item lain: posisi paling bawah yang tersedia */
                if (cy > bestCy) {
                    bestCy = cy;
                    bestCx = cx;
    ...
    function drawClawHeadOnly() {
        ...
        /* Item tertangkap — tampil langsung tanpa bubble di ujung prong */
        if (state.caught && state.caught.img) {
            var iw = state.caught.w;
            var ih = state.caught.h;
            var itemY = ropeEnd + drawH * 0.75;
            ctx.save();
            ctx.drawImage(state.caught.img, cx - iw / 2, itemY, iw, ih);
            ctx.restore();
        }
    }
    ...
    /* Popup menang — klik backdrop untuk tutup, CTA buka URL & tutup popup */
    var popupWinClose = document.getElementById("popupWinClose");
    var ctaBtnEl      = document.getElementById("ctaBtn");
    if (popupWinClose) popupWinClose.addEventListener("click", onPopupClose);
    if (ctaBtnEl) ctaBtnEl.addEventListener("click", function () {
        onPopupClose(); /* tutup popup & reset objek */
    });
